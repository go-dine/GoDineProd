...
Deno.serve(async (req) => {
  try {
    ...
    console.log('Received waiter call webhook:', payload.record.id)
    ...
    // Collect all FCM tokens: primary + multi-device
    const tokens: string[] = []
            ...
            body: JSON.stringify({
              message: {
                ...
                android: {
                  priority: 'high',
                  notification: {
                    channel_id: 'godine_waiter_calls',
                    sound: 'default',
                ...
                data: {
                  waiter_call_id: payload.record.id,
                  table_number: payload.record.table_number,
                  type: 'waiter_call'
                }
              },
            }),
    ...
    console.error('Error sending waiter call notification:', error)
